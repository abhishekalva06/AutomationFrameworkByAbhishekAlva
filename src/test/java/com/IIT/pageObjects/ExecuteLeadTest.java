...
public class ExecuteLeadTest{
	...
	public static void main(String[] args) throws Exception{
		...
		String[] test_cases = new String[]{ "customWords", "AcademicYear", "StudentCategory",
			"HrSettings", "EmployeeAdmission", "Program and Stream creation", "Create_Course_Category",
			"addCourses", "VerifyAndApproveCurricula1",	"StudentAdmission", "AcademicSemester",
			"Cohort", "CohortUpload", "CoursesInCurrentSemester-Verify&Approve",
			"WindowFrame-PreRequisite", "BacklogStudents-FileUpload, Import Logs, Scheduled Jobs",
			"BacklogStudents-Bulk EditNavigation", "PreRegistrationAdminNStudent-Submit Registration Form",
			"WindowFrame-PreRequisite", "SemesterEnrollment- COTnFAApproval", "WindowFrame-PreRequisite", 
			"SemesterEnrollment-DropApproval", "WindowFrame-PreRequisite", "Student'sAddDrop-Interface",
			"WindowFrame-PreRequisite", "Student'sDrop- Interface", "Course_Records"			
		};
		//If you want to run few or one test case just comment the previous lines of code from String[] test_cases to };
		//and uncomment the below line and enter your sheet name
		//String[] test_cases = new String[]{ "customWords"};
		...
		reporter = new ExtentHtmlReporter(project_directory + "/Reports/ReportsOfExecuteLeadTest/" + date_time + "_automation_report.html");
		extent = new ExtentReports();
		extent.attachReporter(reporter);
		elogger = extent.createTest(date_time + "_automation_report");	
		logger = Logger.getLogger("");
		try {	
			fh = new FileAppender(new SimpleLayout(), project_directory+"/logs/LogsOfExecuteLeadTest/"+date_time+"_automation_logs.log");
		...
		file = new FileInputStream(project_directory +"/TestCases/IIT_TestCases.xlsx");
		XSSFWorkbook workbook = new XSSFWorkbook(file);
		for (int i = 0; i < length; i++) {
			XSSFSheet sheet = workbook.getSheet(test_cases[i]);
			row = sheet.iterator();
			while(row.hasNext()) {
